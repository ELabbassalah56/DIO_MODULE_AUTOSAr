...
/* In-case there are no errors */
	...
	if(FALSE == error)
	{
		...
		switch(Dio_PortChannels[ChannelId].Port_Num)
		{
			case 0: Port_Ptr = &PINA_REG;
		        break;
			case 1:	Port_Ptr = &PINB_REG;
		        break;
			case 2:	Port_Ptr = &PINC_REG;
		        break;
			case 3:	Port_Ptr = &PIND_REG;
		        break;
		}
		if(SET_BIT(*Port_Ptr,Dio_PortChannels[ChannelId].Ch_Num))
				{
					status =STD_HIGH;

				}
				else if(CLEAR_BIT(*Port_Ptr,Dio_PortChannels[ChannelId].Ch_Num))
				{
					status=STD_LOW;
